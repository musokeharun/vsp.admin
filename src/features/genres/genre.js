...
import {fetchGenres, selectGenres, updateGenre} from "./genreSlice";
...
const Genre = ({history}) => {
    ...
    const handleUpdate = (id, isActive) => {
        ...
        dispatch(updateGenre(params))
    }

    return (
        <Fragment>
            <PageHeader title={"Genres"} btn={"Add"} btnHandler={e => history.replace("/content/genre/add")}/>
            <div className="table-responsive scrollbar">
                <table className="table table-hover overflow-hidden table-sm">
                    ...
                    <tbody>
                    {
                        !!genres && !!genres.length && genres.map(
                            ({name, desc, isActive, createdAt, image, objectId}) => (
